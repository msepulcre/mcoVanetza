...
McoFac::DataConfirm McoFac::request(const DataRequest& request, DownPacketPtr packet)
{
    ...
    if (router_ && packet) {
        ...
        switch (request.transport_type) {
            case geonet::TransportType::SHB:
                confirm = router_->request(request_shb_mco(request), std::move(packet));
                break;
            case geonet::TransportType::GBC:
                confirm = router_->request(request_gbc_mco(request), std::move(packet));
                break;
            default:
...
void initialize_request_mco(const McoFac::DataRequest& generic, geonet::DataRequest& geonet)
{
...
geonet::GbcDataRequest McoFac::request_gbc_mco(const DataRequest& generic)
{
    assert(router_);
    geonet::GbcDataRequest gbc(router_->get_mib());
    initialize_request_mco(generic, gbc);
    gbc.destination = boost::get<geonet::Area>(generic.destination);
    return gbc;
}

geonet::ShbDataRequest McoFac::request_shb_mco(const DataRequest& generic)
{
    assert(router_);
    geonet::ShbDataRequest shb(router_->get_mib());
    initialize_request_mco(generic, shb);
    return shb;
}

McoFac::PromiscuousHook* McoFac::promiscuous_hook()
{
    return nullptr;
}
